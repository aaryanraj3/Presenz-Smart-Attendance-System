            ...
            <MDBCard>
              <MDBCardBody>
                <MDBCardTitle>
                  ...
                  >
                    <i class="fas fa-qrcode"></i> New QRCode Attendance
                  </MDBBtn>
                  <MDBBtn
                    color="secondary"
                    size="sm"
                    onClick={() => navigate(`/qr/${this.props.data._id}`)}
                  >
                    <i class="fas fa-qrcode"></i> Continue QRCode Attendance
                  </MDBBtn>
                </MDBCardTitle>
                <MDBCardText>
                  <Table
                    columns={columns}
                    dataSource={this.props.parsedD}
                    ...
                    <table class="table table-borderless table-striped table-vcenter">
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            {this.props.data
                              ? this.props.data.schedule.map((a) => (
...
function parseData(data, total) {
  ...
  Object.keys(result).map((key) => {
    arr.push({
      counter: key,
      attended: result[key].length,
      absent: total - result[key].length,
      date: result[key][0].date,
      ratio: (result[key].length * 100) / total,
    });
  });
  console.log(arr);
  return arr;
